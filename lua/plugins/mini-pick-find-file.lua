-- Emacs `C-x C-f` style file picker built on mini.pick: the query IS the path,
-- so you traverse directories by typing instead of confirming each hop.
-- Source: https://github.com/nvim-mini/mini.nvim/discussions/2523
return {
  "nvim-mini/mini.pick",
  -- no mini.icons dependency listed: LazyVim already installs it as part of its UI core,
  -- and naming the new org here would clash with LazyVim's `echasnovski/mini.icons` spec.
  keys = {
    {
      "<leader>fd",
      function()
        require("mini.pick").registry.find_file()
      end,
      desc = "Find File (traverse, cwd)",
    },
    {
      "<leader>f.",
      function()
        require("mini.pick").registry.find_file({ dir = vim.fn.expand("%:p:h") })
      end,
      desc = "Find File (traverse, buffer dir)",
    },
  },
  opts = {},
  config = function(_, opts)
    ...
    local find_file = function(local_opts, start_opts)
      ...
      -- "pressing ~ or / resets query to list home or root directory" feature.
      ...
      local match = function(stritems, inds, query)
        ...
        -- pressing "~" anywhere during a query should immediately bring you home
        ...
        -- meaning the last two chars of the query should be "//"
        if #query > 1 and query[#query] == "/" and query[#query - 1] == "/" then
      ...
      local show = function(buf_id, items_to_show, query)
        ...
        -- 4 because the text is prefixed with icons so 2 for icon and 2 for extra space after text
